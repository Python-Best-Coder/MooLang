...
def generate_id():
    """
    Generate a random 10-digit identifier.

    Returns:
        str: A string consisting of 10 random digits.
    """
...
class Bool:
    def __init__(self, value, inside: Any = False):
    ...
    def run(self, parameters):
        ...
        for line in self.lines:
            ...
            for parameter in self.parameters:
                line = line.replace(
                    parameter,
                    fixed_parameters[self.parameters.index(parameter)]
                )
...
def print_whole_object(object_):
    if isinstance(object_, List):
        lst = []
        for obj in object_.value:
            # If the object is a string, integer, or float, we can print it
            # directly
            while (
                not isinstance((x := print_whole_object(obj)), str)
                or not isinstance((x := print_whole_object(obj)), int)
                or not isinstance((x := print_whole_object(obj)), float)
                or not isinstance((x := print_whole_object(obj)), bool)
            ):
                if (
                    isinstance(x, str)
                    or isinstance(x, int)
...
def tokenize(item: str, inside=None):
    if type(item).__name__ not in [
        "String", "Integer", "List", "Float", "Bool"
    ]:
        item = str(item)
        if item.startswith("[") and item.endswith("]"):
            ...
            for char in item[1:-1]:
                if char == "[":
                    ...
                    depth -= 1
                if char == "," and depth == 0:
                    items.append(tokenize(current.strip(), lst))
                    current = ""
                else:
                    current += char
            if current:
...
def multiply(digit1, digit2):
    ...
    digit1 = (
        tokenize(digit1)
        if not isinstance(digit1, (String, Integer, Float, Bool, List))
        else digit1
    )
    digit2 = (
        tokenize(digit2)
        if not isinstance(digit2, (String, Integer, Float, Bool, List))
        else digit2
    )
    ...
    if digit1 is not None and digit2 is not None:
        if isinstance(digit1, String) and isinstance(digit2, String):
            ...
            raise ValueError(
                f"Unsupported addition between {type(digit1).__name__} " +
                f"and {type(digit2).__name__}"
            )
...
def subtr(digit1, digit2):
    ...
    if isinstance(digit1, Integer) and isinstance(digit2, Integer):
        ...
        raise ValueError(
            f"Unsupported subtraction between {type(digit1).__name__} " +
            f"and {type(digit2).__name__}"
        )
...
def evaluate(expression: str, silent=False):
    ...
    if equal or greater_equal or less_equal or greater or less:
        left = evaluate(
            (equal or greater_equal or less_equal or greater or less)
            .group(1).strip()
        )
        right = evaluate(
            (equal or greater_equal or less_equal or greater or less)
        ...
        left_value = (
            left.value if isinstance(left, (Integer, Float, Bool)) else left
        )
        right_value = (
            right.value if isinstance(right, (Integer, Float, Bool)) else right
        )
    ...
    if floor_division:
        return floor_div(
            evaluate(floor_division.group(1)),
            evaluate(floor_division.group(2))
        )
    ...
    if functions:
        return [
            "function",
            Function(functions.group(1), [], functions.group(2).split(",")),
        ]
    if reassign:
    ...
    if name:
        vname = name.group(1)
        value1 = name.group(5)
        if tokenize(value1) is None:
            while tokenize(value1) is None:
                value1 = evaluate(value1)
        if len(packed) != 0:
            for item in packed:
                if item.name == vname:
                    print(
                        "Error: reassign variable expected (reassign x -> x2)"
                    )
                    quit()
    ...
    if addition:
        ...
        if tokenize(value1) is None or tokenize(value2) is None:
            if tokenize(value1) is None:
                value1 = evaluate(value1)
            if tokenize(value2) is None:
    ...
    for line in data_.splitlines():
        ...
        if type(x).__name__ == "list":
            ...
            if x[0] == "rloop":
                inside.append("rloop")
                insidevalues.append(
                    RangeLoop(evaluate(x[1]).value, evaluate(x[2]).value)
                )
